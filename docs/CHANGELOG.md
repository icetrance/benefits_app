...
## 2026-02-15 — Security & Validation Update

### Security Hardening
- **Validation**: Enforced `@Min(0.01)` on request amounts (prevented negative/zero values).
- **Date Checks**: Blocked future invoice dates in `RequestService`.
- **Budgets**: Implemented strict budget enforcement (Allocated - Spent - Pending > Request Amount).
- **Limits**: Added `maxAmountPerRequest` support for `ExpenseCategory`.

### User & Role Management
- **Auditor Role**: Added `AUDITOR` role and seeded `auditor@expenseflow.local`.
- **Cleanup**: Reduced seeded users to 1 per role (Approver, Employee, Finance, Admin, Auditor).
- **Credentials**: Reset all passwords to `pass` for simplified testing.

---


## 2026-02-12 — Documentation & Standardization

### Documentation
- **Migration**: Moved all project documentation to `docs/` directory.
- **Standardization**: Created `PENDING.md`, `CHANGELOG.md`, and `BUGS.md` for standardized project tracking.
- **Refinement**: Updated `README.md` and `AGENTS.md` to align with the new structure and current project state.
